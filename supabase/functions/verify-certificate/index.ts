...
serve(async (req) => {
  ...
  try {
    ...
    // Method 3: NFT-based verification
      ...
      // Handle both NFT and consensus fallback formats
      if (body.nftTokenId.startsWith('consensus-')) {
        console.log('Using consensus-based verification for fallback token');
        
        // Extract sequence number and query consensus topic
        const sequenceNumber = body.nftTokenId.replace('consensus-', '');
        const topicId = "0.0.6903180";
        ...
        if (!mirrorResponse.ok) {
          throw new Error('Consensus message not found');
        }
        
        const messageData = await mirrorResponse.json();
        const messageBytes = new Uint8Array(atob(messageData.message).split('').map(c => c.charCodeAt(0)));
        const messageText = new TextDecoder().decode(messageBytes);
        const consensusMessage = JSON.parse(messageText);
        
        if (!consensusMessage.ipfsCid) {
          throw new Error('No IPFS CID found in consensus message');
        }
        
        ipfsCid = consensusMessage.ipfsCid;
        console.log('Found IPFS CID from consensus message:', ipfsCid);
